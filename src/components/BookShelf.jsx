...
const BookShelf = () => {
  ...
  useEffect(()=>{
    const handBookElement = document.getElementById("hand-books");
    console.log(handBookElement);
    if (handBookElement) {
      handBookElement.scrollIntoView({behavior:"smooth", block:"nearest", inline:"center"});
    }
  },[]);
  ...
  return (
    ...
    <div className="w-full overflow-x-auto md:container md:mx-8 md:my-4 md:h-full md:flex md:justify-center md:items-center ">
        <ul className="flex gap-5 py-10 md:grid md:grid-cols-5 md:justify-around md:w-full md:h-full">
          {[
            ...
            <li key={index} id={book.id} className="min-w-[200px] flex-shrink-0 md:h-full md:flex-col md:w-full">
              <div className="relative w-full h-60 md:h-[80%]">
                <Image src={book.src} objectFit="cover" layout="fill" alt="image" />
              </div>
